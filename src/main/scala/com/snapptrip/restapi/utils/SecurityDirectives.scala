...
object SecurityDirectives {
  ...
  def authenticate(secretKey: String): Directive1[UserId] =
    headerValueByName("Token")
      ...
      .map(_.toOption.flatMap(decode[AuthTokenContent](_).toOption)) //todo decode is circe method
      .flatMap {
        case Some(result) =>
